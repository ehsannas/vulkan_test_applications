...
class TestManager(object):
    ...
    def gather_all_tests(self):
        ...
        app_names = [f for f in os.listdir(self.apps_directory)]
        # for name in os.listdir(self.root_directory):
        for name in app_names:
            if os.path.isfile(os.path.join(self.bin_directory, name)) and \
                    '.' not in name:
                if self.run_all or name == self.app_name:
                    self.tests[name] = os.path.join(self.bin_directory, name)
        if not self.tests:
            if self.run_all:
