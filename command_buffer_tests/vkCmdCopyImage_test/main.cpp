...
int main_entry(const entry::entry_data* data) {
  ...
  size_t sample_image_data_size = vulkan::GetImageExtentSizeInBytes(
      sample_image_extent, VK_FORMAT_R8G8B8A8_UNORM);
  ...
  {
    // 1. Copy from a 2D color image, with only 1 layer, 1 miplevel and 0
    // offsets in all dimensions to another 2D image created with same config.
